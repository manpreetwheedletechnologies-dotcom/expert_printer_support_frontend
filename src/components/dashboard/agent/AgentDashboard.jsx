...
function buildStatCards(raw) {
  return [
    ...
    {
      label: "Active Chats",
      value: String(raw.active_chats ?? 0),
    ...
    },
    {
      ...
      value: String(raw.resolved_today ?? 0),
      change: "Today",
      positive: true,
      icon: "checkCircle",
    ...
    {
      label: "Avg. Response",
      value: raw.avg_response ?? "—",
      change: "Target: under 5m",
      positive: true,
      icon: "clock",
...
function ChatRequestCard({ req, onAccept, onDecline, accepting }) {
  ...
  return (
    <div
      className={`bg-white border rounded-2xl overflow-hidden shadow-sm transition-opacity ${status === "declined" ? "opacity-60 border-gray-100" : "border-blue-100"}`}
    >
      {/* Top color bar */}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {new Date(
                  req.timestamp || req.createdAt || Date.now(),
                ).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
          ...
          {/* Status badge */}
          <span
            className={`text-xs border px-2.5 py-1 rounded-full font-semibold ${cfg.badge}`}
          >
            {status === "accepted"
              ? "✓ Accepted"
              : status === "declined"
        ...
        <div className="bg-gray-50 rounded-xl p-3 mb-4 space-y-1.5">
          {(req.email || req.visitor?.email) && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-gray-400 uppercase w-14 font-medium">
                Email
              </span>
          ...
          {req.issue && (
            <div className="flex items-start gap-2">
              <span className="text-[10px] text-gray-400 uppercase w-14 font-medium mt-0.5">
                Issue
              </span>
        ...
        {status === "declined" && (
          <div className="flex items-center justify-center gap-2 py-2 bg-gray-50 rounded-xl border border-gray-100">
            <span className="text-gray-400 text-sm">
              Declined — passed to other agents
            </span>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {incomingRequests.filter((r) => r.cardStatus === "pending").length >
              ...
              <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full animate-pulse">
                {
                  incomingRequests.filter((r) => r.cardStatus === "pending")
                    .length
                }
              </span>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {[1, 2].map((i) => (
              ...
              >
                ...
                <div className="flex gap-2">
                  <Skeleton cls="h-10 flex-1 rounded-xl" />
                  <Skeleton cls="h-10 flex-1 rounded-xl" />
                </div>
              </div>
            ))}
          ...
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              {[
                ...
                <th
                  key={h}
                  className="text-left text-xs font-medium text-gray-400 px-5 py-3 uppercase tracking-wider whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {loadingLeads ? (
              ...
              leads.slice(0, 5).map((lead, i, arr) => (
                <tr
                  key={lead.id}
                  className={`hover:bg-gray-50 ${i < arr.length - 1 ? "border-b border-gray-100" : ""}`}
                >
                  <td className="px-5 py-3.5 text-xs text-gray-500 font-mono">
                    {lead.id?.slice(-6)}
                  </td>
                  <td className="px-5 py-3.5 text-xs font-medium text-gray-800 whitespace-nowrap">
                    {lead.customer}
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-xs text-gray-500 min-w-[200px]">
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={`inline-flex text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_STYLES[lead.status] || "bg-gray-100 text-gray-600"}`}
                    >
                      {STATUS_LABEL[lead.status] || lead.status}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-xs text-gray-400 whitespace-pre-line">
...
function TypingIndicator({ label = "Visitor" }) {
  return (
    <div className="flex items-end gap-2">
      <div className="rounded-2xl rounded-bl-md border border-gray-200 bg-white px-3.5 py-2.5 shadow-sm">
      ...
      <span className="text-[11px] text-gray-400 pb-1">
        {label} is typing...
      </span>
    </div>
  );
...
function LiveChatWindow({
  ...
  useEffect(() => {
    ...
    socket.on("chat_history", (h) => {
      ...
      setMessages(
        h.map((m) => ({
          ...
          time: m.createdAt,
          type: m.type || "text",
          fileUrl: buildFileUrl(m.fileUrl),
          fileName: m.fileName || "",
          mimeType: m.mimeType || "",
    ...
    socket.on("receive_message", (msg) => {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: msg._id || `${msg.createdAt}-${Math.random()}`,
          from: msg.sender,
          text: msg.text,
          time: msg.createdAt,
          type: msg.type || "text",
          fileUrl: buildFileUrl(msg.fileUrl),
    ...
    socket.on("typing_indicator", ({ sender, isTyping }) => {
      if (sender === "visitor") setVisitorTyping(isTyping);
    });
    socket.on("admin_joined", (d) => {
      setAdminInChat(true);
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now(),
          from: "system",
          text: d.message || "Admin has joined.",
        },
      ]);
    });
    socket.on("admin_declined", (d) => {
      setAdminRequested(false);
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now(),
          from: "system",
          text: d.message || "Admin unavailable.",
        },
      ]);
    });
    socket.on("chat_closed", () => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now(), from: "system", text: "Chat closed." },
      ]);
    });
    socket.on("visitor_disconnected", () => {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now(), from: "system", text: "Visitor has left the chat." },
      ]);
    });
    ...
    socket.on("visitor_status", ({ roomId, online }) => {
      ...
      if (!online) {
        setMessages((prev) => [
          ...prev,
          { id: Date.now(), from: "system", text: "Visitor went offline." },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
          { id: Date.now(), from: "system", text: "Visitor is back online." },
        ]);
      }
    });
  ...
  const uploadAttachment = async (file) => {
    if (!chat?.roomId || !file) return null;

    const formData = new FormData();
    formData.append("attachment", file);

    const res = await fetch(`${API_BASE}/api/chat/${chat.roomId}/upload`, {
      method: "POST",
      headers: {
  ...
  const handleAttachmentPick = async (e) => {
    ...
    if (!file || chatClosed || !socketRef.current?.connected) return;
    ...
    try {
      ...
      const optimisticMsg = {
        ...
        time: new Date(),
        type: uploaded.type,
        fileUrl: buildFileUrl(uploaded.fileUrl),
      ...
      console.error("[Agent attachment upload failed]", err.message);
      setReplyTo(null);
    } finally {
      setUploading(false);
      e.target.value = "";
 ...
 const send = () => {
  ...
  const requestAdmin = () => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), from: "system", text: "Admin notified." },
    ]);
  };
  ...
  const handleResolve = async () => {
    ...
    onResolve?.(chat.id);
    try {
      await apiFetch(`/api/agent/chats/${chat.id}/resolve`, { method: "POST" });
    } catch {
  ...
  const renderMessageContent = (m, isDark = false) => {
    ...
    if (m.type === "file" && m.fileUrl) {
      return (
        ...
        >
          <Icon
            name="file"
            cls={`w-4 h-4 ${isDark ? "text-white/80" : "text-gray-500"}`}
          />
          <div className="min-w-0">
            ...
            <p
              className={`text-[11px] ${isDark ? "text-white/70" : "text-gray-400"}`}
            >
              {m.mimeType || "file"}
            </p>
          </div>
    ...
    >
      <p
        className={`text-[10px] font-semibold ${
          isDark ? "text-white/90" : "text-blue-700"
        }`}
      >
        {reply.senderName}
      </p>
  ...
  return (
    <div className="flex-1 bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${visitorOnline ? "bg-blue-500 animate-pulse" : "bg-gray-400"}`}
              />
        ...
        <div className="flex items-center gap-2">
          ...
          {chatClosed && (
            <span className="border border-gray-200 text-gray-400 text-sm px-4 py-2 rounded-xl">
              Resolved
            </span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 bg-gray-50/30">
        ...
        {messages.map((m, idx) => {
          if (m.from === "system" || m.from === "divider")
            return (
          ...
          if (m.from === "visitor" || m.from === "customer")
            return (
  <div key={m.id || idx} className="flex flex-col items-start">
    <p className="text-xs text-gray-400 mb-1">{chat.customer}</p>

    <div className="group relative bg-blue-500 text-white px-4 py-3 rounded-2xl rounded-tl-sm max-w-sm shadow-sm">
      {renderReplyPreview(m.replyTo, true)}
      {renderMessageContent(m, true)}
      ...
      <button
        ...
        onClick={() =>
          setReplyTo({
            id: m.id,
            text: m.fileName || m.text || "Attachment",
            type: m.type || "text",
            senderName: chat.customer || "Visitor",
          })
        }
          ...
          if (m.from === "admin")
            return (
              <div key={m.id || idx} className="flex flex-col items-end">
                <p className="text-xs text-purple-500 font-semibold mb-1 text-right">
                  {m.senderName || "Admin"}
                </p>
                <div className="bg-purple-500 text-white text-sm rounded-2xl rounded-tr-sm px-4 py-3 max-w-sm leading-relaxed">
                  {renderMessageContent(m, true)}
                </div>
              </div>
            );
          if (m.from === "bot")
            return (
              <div key={m.id || idx} className="flex flex-col items-start">
                ...
                <div className="bg-gray-100 text-gray-700 text-sm rounded-2xl px-4 py-3 max-w-sm">
                  {renderMessageContent(m, false)}
                </div>
              </div>
            );
          return (
            <div key={m.id || idx} className="flex flex-col items-end">
             ...
             <div className="bg-white border border-gray-100 shadow-sm text-gray-700 text-sm rounded-2xl rounded-tr-sm px-4 py-3 max-w-sm leading-relaxed">
  {renderReplyPreview(m.replyTo, false)}
  {renderMessageContent(m, false)}
</div>
            </div>
          );
        })}
        {visitorTyping && <TypingIndicator label={chat.customer || "Visitor"} />}
        <div ref={bottomRef} />
      </div>

      <div className="px-6 py-4 border-t border-gray-100 bg-white">
        {replyTo && (
  <div className="mb-3 flex items-start justify-between rounded-xl border border-blue-200 bg-blue-50 px-3 py-2">
    <div className="min-w-0">
    ...
    >
      ✕
        ...
        <div
          className={`flex items-center gap-3 border rounded-2xl px-4 py-3 transition-all ${chatClosed ? "border-gray-100 bg-gray-50" : "border-gray-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100"}`}
        >
          <input
            value={input}
          ...
          <button
            ...
            disabled={chatClosed || uploading || !socketRef.current?.connected}
            className="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center hover:bg-gray-50 disabled:opacity-50"
            title="Attach file"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828L18 10.828a4 4 0 10-5.656-5.656L6.757 10.757a6 6 0 108.486 8.486L20 13"
              />
            </svg>
          </button>
          ...
          <button
            onClick={send}
            disabled={chatClosed || !input.trim()}
...
function LiveChatsView({
  ...
  const fetchChats = useCallback(async () => {
    try {
      ...
      const normalised = (data.chats || [])
        .map(normaliseChat)
        .filter((c) => c.status === "active" || c.status === "closed");

      setChats(normalised);

      // Hydrate sidebar online/offline state from backend data
      setVisitorOnlineMap((prev) => {
        const next = { ...prev };
        normalised.forEach((chat) => {
          next[chat.roomId] = Boolean(chat.visitorOnline);
        });
        return next;
      });
    } catch (err) {
  ...
  const handleResolve = (chatId) => {
    ...
    setActiveChat((prev) =>
      prev?.id === chatId ? { ...prev, status: "closed" } : prev,
    );
  };
            ...
            chats.map((c) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span
                    className={`text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ml-1 transition-all ${
                      visitorOnlineMap[c.roomId]
                        ? "bg-blue-500 text-white"
                        : "border border-gray-200 text-gray-400 bg-white"
                    }`}
                  >
                    {visitorOnlineMap[c.roomId] ? "Active" : "Inactive"}
                  </span>
                </div>
...
function LeadsView({ search }) {
  ...
  const updateStatus = async (id, status) => {
    ...
    try {
      await apiFetch(`/api/agent/leads/${id}/status`, {
        method: "PATCH",
  ...
  const filtered = leads.filter((l) =>
    [l.id, l.customer, l.email, l.printer, l.issue, l.status].some((v) =>
      String(v || "")
        .toLowerCase()
  ...
  if (loading)
    return (
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
        {[1, 2, 3, 4, 5].map((i) => (
          <Skeleton key={i} cls="h-10 w-full" />
        ))}
      </div>
    );
          ...
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              {[
                ...
                <th
                  key={h}
                  className="text-left text-xs font-medium text-gray-400 px-5 py-4 uppercase tracking-wider whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-5 py-12 text-center">
                  ...
                  {!search && (
                    <p className="text-xs text-gray-300 mt-1">
                      Accept a chat request to create your first lead.
                    </p>
                  )}
                </td>
              </tr>
              ...
              filtered.map((lead, i, arr) => (
                ...
                >
                  <td className="px-5 py-4 text-xs text-gray-500 font-mono">
                    {String(lead.id || "").slice(-6) || "—"}
                  </td>
                  <td className="px-5 py-4 text-xs font-medium text-gray-800 whitespace-nowrap">
                    {lead.customer || "—"}
                  </td>
...
export default function AgentDashboard({ user, onLogout }) {
  ...
  const loadWaitingChats = useCallback(async () => {
    try {
      ...
      setIncomingRequests((prev) => {
        ...
        const actedOn = prev.filter(
          (p) => p.cardStatus === "accepted" || p.cardStatus === "declined",
        );
      ...
      console.error(
        "[AgentDashboard] Failed to load waiting chats:",
        err.message,
      );
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on("chat_queue_update", (data) => {
      ...
      if (data.action === "accepted") {
        setIncomingRequests((prev) =>
          prev.map((r) =>
            r.roomId === data.roomId ? { ...r, cardStatus: "accepted" } : r,
          ),
        );
        setAccepting(null);
      }
      ...
      if (data.action === "visitor_left") {
        setIncomingRequests((prev) =>
          prev.filter((r) => r.roomId !== data.roomId),
        );
        setAccepting(null);
      }
  ...
  const handleAccept = async (req) => {
    ...
    try {
      ...
      setIncomingRequests((prev) =>
        prev.map((r) =>
          r.roomId === req.roomId ? { ...r, cardStatus: "accepted" } : r,
        ),
      );
      ...
      setIncomingRequests((prev) =>
        prev.map((r) =>
          r.roomId === req.roomId ? { ...r, cardStatus: "declined" } : r,
        ),
      );
    } finally {
      setAccepting(null);
  ...
  const handleDecline = (roomId) => {
    ...
    setIncomingRequests((prev) =>
      prev.map((r) =>
        r.roomId === roomId ? { ...r, cardStatus: "declined" } : r,
      ),
    );
  };
